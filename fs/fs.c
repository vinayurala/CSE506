...
void *crypto_page = (void *)(UTEMP + 3 * PGSIZE);
...
// Allocate a specific block, we need this to store 
// encryption and decryption keys.
int 
alloc_crypto_block(int block_num)
{
...
check_encrypt()
{
	uint64_t *pg_temp = (uint64_t *)crypto_page;
	int r, i, num_tries = 1;

	if((r = sys_page_alloc(0, crypto_page, PTE_U | PTE_W | PTE_P)))
	{
		panic("\nSys_page_alloc failure in check_encrypt. Reason: %e\n", r);
		return -1;
	} 

	if((r = sys_page_alloc(0, (void *)pg_temp, PTE_U | PTE_W | PTE_P)))
	{
		panic("\nSys_page_alloc failure in check_encrypt. Reason: %e\n", r);
		return -1;
	} 

	ide_read(CRYPTO_BLOCK * BLKSECTS, pg_temp, BLKSECTS);
	if(pg_temp[0] == 0)
		return 0;
	...
	else
	{
		...
		cprintf("Encrypted disk found. Please enter the password to unlock the disk (Try: %d/3)\n", num_tries);
		while(1)
		{
			if(num_tries > 3)
			{
				panic("Password tries exceeded, panicking!!\n");		      
				return -1;
			}
			...
			if(pg_temp[0] != JEDI)
				cprintf("\nWrong password! Re-enter password (Try: %d/3)\n", ++num_tries); 
			else 
				break;
		}
			cprintf("\nPassword accepted, continuing......\n");
...
fs_init(void)
{
	...
	super = diskaddr(1);
	if(r1)
		transparent_disk_decrypt(1, super);
	...
	if(!r1)
	{
		cprintf("\nDisk not encrypted yet, encrypting it....\n");
		r = full_disk_encrypt();
		if(r)
		{
			panic("\nDisk encryption error: %e\n", r);
			return;
		}
...
{
	// LAB 5: Your code here.
	//cprintf("entered file_block_walk\n");
	uint32_t *ptr;
	int blocknum;
	...
	if(!(f->f_indirect))
	{
		...
		{
			cprintf("\nNo more blocks left to allocate\n");
			return -E_NO_DISK;
		}
...
{
	// LAB 5: Your code here.
	//cprintf("file_get_block entered\n");
	uint32_t *ppdiskbno;
	int blocknum;
