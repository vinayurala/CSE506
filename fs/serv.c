...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if (size > PGSIZE)
		size = PGSIZE;
	if ((rd = openfile_lookup(envid, req->req_fileid, &of)) < 0)
		return rd;
	if ((rd = file_read(of->o_file, (void *)ret->ret_buf, size, of->o_fd->fd_offset )) < 0)
		return rd;
	of->o_fd->fd_offset = of->o_fd->fd_offset + rd;
	return rd;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int rd;
	struct OpenFile *of;
	struct OpenFile **ofp = &of;
	if((rd = openfile_lookup(envid,req->req_fileid,&of)) < 0)
		return rd;
	if((rd = file_write((*ofp)->o_file,req->req_buf,req->req_n,(*ofp)->o_fd->fd_offset)) < 0)
		return rd;
	(*ofp)->o_fd->fd_offset = of->o_fd->fd_offset + rd;
	return rd;
