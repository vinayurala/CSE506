...
env_setup_vm(struct Env *e)
{
	...
	//pdpe_t *pdpe;
	//pde_t *pgdir;
	pdpe_t *env_pdpe = NULL;
	pde_t *env_pgdir = NULL;
	pdpe_t *boot_pdpe = NULL;
	...
	if(!(p1 = page_alloc(0)))
		return -E_NO_MEM;
	p1->pp_ref++;
	env_pdpe = page2kva(p1);
	boot_pdpe = KADDR(PTE_ADDR(boot_pml4e[PML4(UTOP)]));
	if(!(p2 = page_alloc(0)))
		return -E_NO_MEM;
	p2->pp_ref++;
	env_pgdir = page2kva(p2);
	boot_pgdir = KADDR(PTE_ADDR(boot_pdpe[PDPE(UTOP)]));
	for(i = PDX(UTOP); i < NPDENTRIES; i++)
		env_pgdir[i] = boot_pgdir[i]; 

	env_pdpe[PDPE(UTOP)] = PADDR(env_pgdir) | PTE_U | PTE_P | PTE_W;
	e->env_pml4e[0] = PADDR(env_pdpe) | PTE_U | PTE_P | PTE_W;
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (i = 0; i < len;i+=PGSIZE)
	{
		...
		res = page_insert(e->env_pml4e, page_var, va + i, PTE_U | PTE_W);
		if(res == -E_NO_MEM)
		{
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	{
		panic("load_icode failed: ELF header not valid!!\n");
		return;
	}
	...
	for(; ph < eph; ph++)
	{
		if(ph->p_type == ELF_PROG_LOAD)
		{
			...
			{
				panic("load_icode failed: Binary cannot fit in memory!! (ph->p_filesz > ph->p_memsz)\n");
				return;
			}
	...
	if(!page_var)
	{
		panic("load_icode ---> page_alloc: %e\n", -E_NO_MEM);
		return;
	}
	ins_error = page_insert(e->env_pml4e, page_var, (void *)USTACKTOP - PGSIZE, PTE_U | PTE_W);
	if(ins_error == -E_NO_MEM)
	{
		panic("load_icode ---> page_insert: %e\n", ins_error);
		return;
	}
	e->env_tf.tf_rip = elfhdr->e_entry;
//	cprintf("elf->e_entry = %x",elfhdr->e_entry);
	return;
}
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
	...
	if((type == ENV_TYPE_FS) || (type == ENV_TYPE_CRYPT))
