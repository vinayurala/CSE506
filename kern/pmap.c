...
x64_vm_init(void)
{
	...
	boot_map_segment(pml4e, UPAGES, n, PADDR(pages), PTE_U);
	...
	boot_map_segment(pml4e, UENVS, n, PADDR(envs), PTE_U);
	...
	// Your code goes here:
        /* boot_map_segment(pml4e, KSTACKTOP-KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_W) */;
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here: 
	boot_map_segment(pml4e, KERNBASE, 0xFFFFFFFF - KERNBASE, 0, PTE_W);
...
mem_init_mp(void)
{
	...
	for(i = 0; i < NCPU; i++)
	{
		...
		boot_map_segment(pml4e, (kstacktop_i - KSTKSIZE), KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
      	}
...
page_init(void)
{
	...
	for (i = 1; i < npages; i++) {
		...
		if(((page2pa(&pages[i])) >= IOPHYSMEM) && ((page2pa(&pages[i])) < PADDR(boot_alloc(0))))	
			continue;
		else if ((page2pa(&pages[i]) >= PADDR(BOOT_PAGE_TABLE_START)) && ((page2pa(&pages[i])) < PADDR(BOOT_PAGE_TABLE_END)))
			continue;
...
page_alloc(int alloc_flags)
{
	...
	size_t i;
	struct Page *page_var = page_free_list;
	if(alloc_flags | ALLOC_ZERO)
	{
		if(page_free_list)
	       	{
	      		memset(page2kva(page_var), 0 , PGSIZE);
                        page_free_list = page_free_list->pp_link;
			page_var->pp_link = NULL;
      	       		return page_var;
               	}
       		else
			return NULL;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	{
		panic("\nSize to be allocated is greater than MMIOLIM!\n");
		return (void *)base;
	} 
	boot_map_segment(pml4e, base, size, pa, PTE_P | PTE_W | PTE_PCD | PTE_PWT);
	base += size;
