...
copy_shared_pages(envid_t child)
{
        ...
        int pn, end_pn;
        uint64_t va;
        //for(pn = PPN(UTEXT), va = UTEXT; pn < PPN(UTOP) && va < UTOP; va += PGSIZE)
	for(va = UTEXT;va < UTOP; va += PGSIZE)
        {
                if(!((vpml4e[VPML4E(va)])&&(vpde[VPDPE(va)]) && (vpd[VPD(va)]  )))
                {
                       // pn += NPTENTRIES;
                        continue;
                }
                ...
                //{
                        if((vpt[VPN(va)] & PTE_SHARE) != 0)
                        {
                                //cprintf("before page_map in lib/spawn.c\n");
                                sys_page_map(0, (void *)va, child, (void *)va, vpt[VPN(va)]&PTE_USER);
